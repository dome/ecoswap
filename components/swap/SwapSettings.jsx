    ...
    <>
      <div className="settings">
            ...
            <div className="gas-section">
              <div className="section-title">
                ...
                <div className="title-value">
                  {" "}
                  -{" "}
                  {typeof settings.gas[chain.id] === "number"
                    ? `custom ${settings.gas[chain.id]}`
                    : chain.gasPrice[settings.gas[chain.id]]}{" "}
                  gwei
                </div>
              </div>
              <div className="gas-controls">
                <div
                  className="gas-switch"
                  data-checked={settings.gas[chain.id] === "slow"}
                  onClick={() => updateGas("slow")}
                ></div>
                <div className="gas-label">Slow</div>
                <div
                  className="gas-switch"
                  data-checked={settings.gas[chain.id] === "default"}
                  onClick={() => updateGas("default")}
                ></div>
                <div className="gas-label">Default</div>
                <div
                  className="gas-switch"
                  data-checked={settings.gas[chain.id] === "fast"}
                  onClick={() => updateGas("fast")}
                ></div>
              ...
              <div className="router" key={router}>
                <div className="router-section router-title">
                  <img
                    ...
                    src={`/routers/${router}.svg`}
                  ></img>
                  {settings.routers[router].name}
                </div>
                <div className="router-section">
                  <div className="router-status">
                    {settings.routers[router].enabled ? "Enabled" : "Disabled"}
                  </div>
                  <label className="switch">
                    <input
                      type="checkbox"
                      checked={settings.routers[router].enabled}
                      onChange={() => toggleRouter(router)}
                    ></input>
        ...
        <div className="section">
          {chain.id == "0x60" ? (
            <div className="links">
              <a href="https://kkub-otp.bitkubchain.com/" target="_blank">
                Please KYC your address with this link before UNWRAP KKUB :
              </a>
              <br />
